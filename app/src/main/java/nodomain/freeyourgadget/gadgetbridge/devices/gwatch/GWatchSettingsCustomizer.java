...
import static nodomain.freeyourgadget.gadgetbridge.devices.gwatch.GWatchConstants.PREF_BANGLEJS_ACTIVITY_FULL_SYNC_STOP;
import static nodomain.freeyourgadget.gadgetbridge.devices.gwatch.GWatchConstants.PREF_BANGLEJS_ACTIVITY_FULL_SYNC_TRIGGER;
...
public class GWatchSettingsCustomizer implements DeviceSpecificSettingsCustomizer {
    private ProgressDialog activityFullSyncDialog;

    final GBDevice device;

    public GWatchSettingsCustomizer(final GBDevice device) {
        this.device = device;
    }
    ...
    public static final Creator<GWatchSettingsCustomizer> CREATOR = new Creator<GWatchSettingsCustomizer>() {
        @Override
        public GWatchSettingsCustomizer createFromParcel(final Parcel in) {
            final GBDevice device = in.readParcelable(GWatchSettingsCustomizer.class.getClassLoader());
            return new GWatchSettingsCustomizer(device);
        }

        @Override
        public GWatchSettingsCustomizer[] newArray(final int size) {
            return new GWatchSettingsCustomizer[size];
        }
    };
}
